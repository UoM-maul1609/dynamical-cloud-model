...
if (clouds)
    ...
    sigma_s_clouds(ind)=pi./2.*100e6.*10e-6.^2.*(z(ind)-lay1)./(lay2-lay1);
    if shortwave
...
tau(kp+1)=1e-2;
for k=1:kp+1
    dtau(k)=sigma_tot(k)*dz;
end
for k=kp:-1:1
    tau(k)=tau(k+1)+dtau(k+1);
end
