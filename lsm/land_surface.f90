    ...
    module lsm
	use numerics_type
    use numerics, only : romb, tridiagonal
    implicit none
        ...
        type lsmg
            ...
            real(wp), dimension(:,:,:), allocatable :: rad_power, t, wg
            real(wp), dimension(:,:), allocatable :: tsurf
            real(wp), dimension(:), allocatable :: sflux_l	
            real(wp), dimension(:), allocatable :: sz,szn	, dsz, dszn
            real(wp), dimension(:), allocatable :: a,b,c,r,u, pscs, b1, wgs, wfc, &
                                                 phi_ps, kgs
		...
		real(wp), parameter :: r_gas=8.314_wp, ma=29e-3_wp, ra=r_gas / ma, &
								t_stp=288._wp, p_stp=101300._wp, navog=6.022e23_wp, &
								sflux=1370._wp, mu1=1._wp/sqrt(3._wp)
        ...
        type namelist_lsm
        	...
        	real(wp) :: lat_ref,lon_ref, tinit, wg_pc_init
            ...
            real(wp), dimension(20) :: lambda_read_s, soil_thickness
        ...
        type lsm_vars
            ! soil parameters
            real(wp), dimension(nsoil) :: b=[4.05, 4.38, 4.90, 5.30, 5.39, 7.12, &
                                            7.75, 8.52, 10.40, 10.40, 11.40]
            real(wp), dimension(nsoil) :: wgs=[0.395, 0.410, 0.435, 0.485, 0.451, &
                                            0.420, 0.477, 0.476, 0.426, 0.492, 0.482]
            real(wp), dimension(nsoil) :: wfc=[0.135, 0.150, 0.195, 0.255, 0.240, 0.255, &
                                            0.322, 0.325, 0.310, 0.370, 0.367]
            real(wp), dimension(nsoil) :: wwilt=[0.068,0.075,0.114,0.179,0.155, &
                                            0.175,0.218,0.250,0.219,0.283,0.286]
            real(wp), dimension(nsoil) :: phips=[-12.1,-9.0,-21.8,-78.6,-47.8,-29.9, &
                                            -35.6,-63.0,-15.3,-49.0,-40.5]
            real(wp), dimension(nsoil) :: kgs=[1.76e-4, 1.56e-4, 3.41e-5, 7.20e-6, & 
                        7.00e-6, 6.30e-6, 1.70e-6, 2.50e-6, 2.20e-6, 1.00e-6, 1.30e-6]
            real(wp), dimension(nsoil) :: pscs=[1.47e6, 1.41e6, 1.34e6, 1.27e6, 1.21e6, &
        ...
        real(wp), parameter :: rv=461.5_wp,eps1=ra/rv,&
                        cp=1005._wp, cw=4190._wp, ttr=273.15_wp, lv=2.5e6_wp, &
                            ra_on_cp=ra/cp
		...
		subroutine allocate_and_set_lsm(ip,jp,kp, &
			...
			real(wp), dimension(:), allocatable, intent(inout) :: a,b,c,r,u,  sz, szn, &
			        dsz, dszn, pscs, b1, wgs, wfc,phi_ps, kgs
			real(wp), dimension(:,:,:), allocatable, intent(inout) :: t,wg
			real(wp), dimension(:,:), allocatable, intent(inout) :: tsurf
			real(wp), dimension(skp_nm), intent(in) :: soil_thickness
			character (len=charlen), dimension(stl), intent(in) :: soil_types
			real(wp), intent(in) :: tinit, wg_pc_init, tsurf1
...
! if the pe is not being used in the cartesian topology, do not use here
		...
		subroutine soil_solver(ip,jp,skp,l_h, r_h, tdend,a,b,c,r,u, pscs,b1,wgs,phi_ps, &
			...
			real(wp), dimension(1-l_h:skp+r_h,1-l_h:jp+r_h,1-l_h:ip+r_h), &
			    intent(inout) :: t,wg
			real(wp), dimension(1-l_h:jp+r_h,1-l_h:ip+r_h), &
			    intent(inout) :: tsurf
			real(wp), dimension(1-l_h:jp+r_h,1-l_h:ip+r_h), &
			...
			integer(i4b) :: i,j,k
			real(wp) :: alpha_i, beta_i, beta_ip1, kval, flux1, flux2, dp05, dm05, &
			    gamma_i, phip, kp05, km05
			real(wp), dimension(1-l_h:skp+r_h) :: dg, kg, ks, pgcg
            ...
            do i=1,ip
                do j=1,jp
                    ...
                    do k=1-l_h,skp+r_h
                        ...
                        phip=phi_ps(k)*(wgs(k)/max(wg(k,j,i),1.e-8_wp))**b1(k)
                        ks(k)=max(418._wp*exp(-log10(abs(phip)-2.7_wp)),0.172_wp)
                        ! equation 8.94 - note that the soil has air gaps, which
                        !  are neglected in the first term
                        pgcg(k)=(1._wp-wgs(k))*pscs(k)+wg(k,j,i)*4.2e6_wp
                    enddo   
                            
                    ! first layer, k=1
                    kp05=(ks(2)+ks(1))*0.5_wp
                    km05=(ks(1)+ks(0))*0.5_wp
                    alpha_i=dt*kp05/(2._wp*pgcg(1)*dsz(0)*dszn(1))
                    ...
                    c(1)=-alpha_i
                    ! result
                    r(1)=(1._wp-alpha_i)*t(1,j,i)+ &
                                alpha_i*t(2,j,i)+beta_i*flux1
                            
                    
                    ! internal elements from 1 to tdend-1
                    do k=2,tdend-1
                        kp05=(ks(k+1)+ks(k))*0.5_wp
                        km05=(ks(k)+ks(k-1))*0.5_wp
                        alpha_i=dt*kp05/(2._wp*pgcg(k)*dsz(k-1)*dszn(k))
                        beta_i=dt*km05/(2._wp*pgcg(k)*dsz(k-1)*dszn(k-1))
                        ! diagonal
                        b(k)=1._wp+alpha_i+beta_i
                        ! super-diagonal
                        c(k)=-alpha_i
                        ! sub-diagonal
                        a(k-1)=-beta_i
                        ! result
                        r(k)=(1._wp-alpha_i-beta_i)*t(k,j,i)+ &
                    ...
                    do k=1-l_h,skp+r_h
                        ...
                        dg(k)=-b1(k)*kgs(k)*phi_ps(k)/ &
                                wgs(k)*(wg(k,j,i)/wgs(k))**(b1(k)+2._wp)
                        kg(k)=kgs(k)*(wg(k,j,i)/wgs(k))**(2._wp*b1(k)+3._wp)
                    enddo   
                    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   
            
                    ! first layer, k=1
                    dp05=(dg(2)+dg(1))*0.5_wp
                    dm05=(dg(1)+dg(0))*0.5_wp
                    alpha_i=dt*dp05/(2._wp*dsz(0)*dszn(1))
                    beta_i=dt/dsz(0)
                    ...
                    c(1)=-alpha_i
                    ! result
                    r(1)=(1._wp-alpha_i)*wg(1,j,i)+ &
                                alpha_i*wg(2,j,i)-beta_i*flux2+gamma_i

                    ! now set the RHS
                    do k=2,tdend-1
                        dp05=(dg(k+1)+dg(k))*0.5_wp
                        dm05=(dg(k)+dg(k-1))*0.5_wp
                        alpha_i= dt*dp05/(2._wp*dsz(k-1)*dszn(k))
                        beta_i = dt*dm05/(2._wp*dsz(k-1)*dszn(k-1))
                        ...
                        a(k-1)=-beta_i
                        ! result
                        r(k)   = alpha_i*wg(k+1,j,i)+(1._wp-alpha_i-beta_i)*wg(k,j,i)+ &
...
!            print *,tsurf(1,1),t(1,1,1), wg(1,1,1)
        ...
        real(wp), dimension(1-l_h:jp+r_h, 1-l_h:ip+r_h), intent(in) :: &
            tsurf, w1
        real(wp), intent(in), dimension(-r_h+1:kp+r_h,-r_h+1:jp+r_h,-l_h+1:ip+r_h) :: &
            th, q, u,v
            
        real(wp), intent(inout), dimension(-r_h+1:kp+r_h,-r_h+1:jp+r_h,-l_h+1:ip+r_h) :: &
            sth
            
        real(wp), intent(inout), &
            dimension(-r_h+1:kp+r_h,-r_h+1:jp+r_h,-l_h+1:ip+r_h,nq) :: sq
            
        real(wp), dimension(1-l_h:jp+r_h, 1-l_h:ip+r_h), intent(inout) :: hf, lef
        real(wp), intent(in), dimension(-r_h+1:kp+r_h) :: rhoan, thetan, zn,z,dz
        real(wp), intent(in) :: z0, z0th, psurf, dt, wfc

        ! locals
        integer(i4b) :: i,j
        real(wp), dimension(-r_h+1:jp+r_h,-l_h+1:ip+r_h) :: vmag,rib,gm,gh, &
                                                                ustar,thstar
        real(wp) :: thcalc, qstar, qv0
            
        do i=1,ip
            do j=1,jp
                thcalc = tsurf(j,i)*(1.e5_wp/psurf)**(ra_on_cp)
                ...
                if(rib(j,i) .le. 0._wp) then
                    gm(j,i)=1._wp-9.4_wp*rib(j,i)/ &
                        (1._wp+70._wp*kvon**2* &
                        sqrt(abs(rib(j,i)*z(1)/z0))/(log(z(1)/z0))**2)
                    gh(j,i)=1._wp-9.4_wp*rib(j,i)/ &
                        (1._wp+50._wp*kvon**2* &
                        sqrt(abs(rib(j,i)*z(1)/z0))/(log(z(1)/z0))**2)
                else
                    gm(j,i)=1._wp/(1._wp+4.7_wp*rib(j,i))**2
                    gh(j,i)=1._wp/(1._wp+4.7_wp*rib(j,i))**2
                endif
            ...
            enddo
        enddo
        !print *,sum((lef(1:jp,1:ip)))/real(ip*jp,wp),sum((hf(1:jp,1:ip)))/real(ip*jp,wp), &
